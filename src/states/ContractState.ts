...
export class ContractState extends Phaser.Scene {
  ...
  private generateContracts() {
    ...
    for (let i = 0; i < numContracts; i++) {
      ...
      const contract: Contract = {
        ...
        knownDangers: this.getRandomDangers(dangers, Math.floor(Math.random() * 3) + 1),
  ...
  private getRandomDangers(dangers: string[], count: number): string[] {
    const shuffled = [...dangers].sort(() => Math.random() - 0.5);
